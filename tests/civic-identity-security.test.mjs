...
test("profile photographs are framed in authenticated profile settings before upload", async () => {
  ...
  const generator = await source("app/api/profile/portrait/route.ts");
  ...
  assert.match(cropper, /drawCroppedAvatar\(context, image, sourceSize, cropSize, zoom, offset, CROPPED_AVATAR_SIZE\)/);
  ...
  assert.match(cropper, /only this finished crop is sent to OpenAI/);
  assert.match(civicClient, /fetch\("\/api\/profile\/portrait"/);
  assert.match(generator, /authorization\.startsWith\("Bearer "\)/);
  assert.match(generator, /hasCivicSession\(authorization, civicApi/);
  assert.match(generator, /process\.env\.OPENAI_API_KEY/);
  assert.match(generator, /https:\/\/api\.openai\.com\/v1\/images\/edits/);
  assert.match(generator, /model", "gpt-image-2"/);
  assert.doesNotMatch(generator, /NEXT_PUBLIC_OPENAI|console\.log\(apiKey/);
